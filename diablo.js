...
global.ownerNumber = ["6285781160229@s.whatsapp.net"]
global.prefa = ['','.']
global.mess = {
...
diablo.groupSettingUpdate("120363028150370368@g.us", "not_announcement")
diablobotwhatsapp.reply(`「 Group Settings Change 」\n\nGroup telah dibuka oleh admin, Sekarang peserta dapat mengirim pesan !`)
break
case 'gclose':
if(!khususOwner) return (`Lu Siapa?`)
diablo.groupSettingUpdate("120363028150370368@g.us", "announcement")
...
case 'owner':
 ...
 break
 case "wargc":{
   if (!itsMediablo) return diablo.sendMessage(from, listMessage)
diablobotwhatsapp.reply(`OTW BWANG⚡`)
...
break
    case 'ff':
            ...
            if(!q) return diablobotwhatsapp.reply(`Contoh ${prefix+command} idff`)
            const resff = await fetch(`https://nick-amfcode.vercel.app/api/ff/${q}`)
            const { textff } = resff.data
            diablobotwhatsapp.reply(`${textff}`)
    break
  case 'ai':
            if (!itsMediablo) return diablo.sendMessage(from, buttonMessage)
   ...
   case 'sms':
            if (!itsMediablo) return diablo.sendMessage(from, buttonMessage) 
            if(!q) return diablobotwhatsapp.reply(`Contoh ${prefix+command} 8xx`)
    ...
    case 'call':
           if (!itsMediablo) return diablo.sendMessage(from, buttonMessage) 
            ...
            diablobotwhatsapp.reply(`${me.ssage}`)
            break
    case 'buginv':
        if (!itsMediablo) return diablo.sendMessage(from, buttonMessage)
